...
class FeatureContext implements Context
{
    ...
    {
        $pieCommand = array_merge(['bin/pie'], explode(' ', $command));

        $proc = (new Process($pieCommand))
            ->mustRun();
    ...
    public function theLatestVersionShouldHaveBeenDownloaded(): void
    {
        ...
        Assert::regex(
            $this->output,
            '#Found package: asgrim/example-pie-extension:\d+\.\d+\.\d+ which provides ext-example_pie_extension#',
        );

        Assert::regex(
            $this->output,
            '#Extracted asgrim/example-pie-extension:\d+\.\d+\.\d+ source to: .+/asgrim-example-pie-extension-[a-z0-9]+#',
        );
    }
    ...
    public function versionOfTheExtensionShouldHaveBeen(string $version)
    {
        ...
        Assert::contains(
            $this->output,
            'Found package: asgrim/example-pie-extension:' . $version . ' which provides ext-example_pie_extension',
        );
    }
}
